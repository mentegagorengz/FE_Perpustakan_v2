...
export default function Header() {
  ...
  return (
    ...
    >
          ...
          {isAuthenticated ? (
            <>
              <span className="text-[16px] text-white/70">
                Halo, <span className="text-white">{user?.nama ?? "User"}</span>
              </span>
      ...
      {showToast && (
        <div role="status" className="fixed right-5 top-5 rounded-sm bg-secondary px-5 py-3 text-sm text-white shadow-[var(--shadow-overlay)]">
