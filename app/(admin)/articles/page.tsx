    ...
    <div className="min-h-screen bg-cream p-4 font-sans sm:p-6 lg:p-10">
      <div className="mb-8 flex flex-col items-start gap-5 border-b border-main-border pb-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-secondary px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-secondary-hover sm:w-auto"
        >
      ...
      {ui.showForm && (
        <AccessibleDialog titleId="article-form-title" onClose={ui.resetForm} className="flex max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-lg bg-cream shadow-[var(--shadow-overlay)]">
            <div className="flex items-center justify-between border-b border-main-border bg-white p-6">
              ...
              <button type="button" onClick={ui.resetForm} aria-label="Tutup formulir artikel" className="flex h-11 w-11 items-center justify-center rounded-md text-main-text/50 transition-colors hover:bg-surface hover:text-main-text">
                <X aria-hidden="true" className="h-5 w-5" />
              </button>
            </div>
            <form onSubmit={ui.handleActionSubmit} className="space-y-4 overflow-y-auto p-6">
              {ui.articleError && <div role="alert" className="rounded-md border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-700">{ui.articleError instanceof Error ? ui.articleError.message : "Gagal memproses artikel."}</div>}
              <div>
                <label htmlFor="article-title" className="mb-1 block text-sm font-medium text-main-text/70">Judul artikel</label>
                <input id="article-title" name="title" required value={ui.form.title} onChange={(e) => ui.form.setTitle(e.target.value)} className="w-full rounded-md border border-main-border px-4 py-2.5 text-sm outline-none focus:border-secondary" />
              </div>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="article-cover" className="mb-1 block text-sm font-medium text-main-text/70">Gambar sampul</label>
                  <input id="article-cover" name="cover" type="file" accept="image/*" onChange={(e) => ui.form.setSelectedFile(e.target.files?.[0] || null)} className="w-full rounded-md border border-main-border bg-white px-4 py-2 text-xs file:mr-4 file:rounded-md file:border-0 file:bg-secondary file:px-4 file:py-2 file:text-xs file:font-medium file:text-white" />
                </div>
                <div>
                  <label htmlFor="article-status" className="mb-1 block text-sm font-medium text-main-text/70">Status publikasi</label>
                  <select id="article-status" name="status" value={ui.form.isPublished ? "true" : "false"} onChange={(e) => ui.form.setIsPublished(e.target.value === "true")} className="w-full rounded-md border border-main-border px-4 py-2.5 text-sm outline-none focus:border-secondary">
              ...
              <div>
                ...
                <textarea id="article-content" name="content" required value={ui.form.content} onChange={(e) => ui.form.setContent(e.target.value)} rows={8} className="w-full resize-none rounded-md border border-main-border px-4 py-3 text-sm outline-none focus:border-secondary" />
              </div>
      ...
      {ui.showDeleteConfirm && (
        <AccessibleDialog titleId="delete-article-title" descriptionId="delete-article-description" onClose={() => ui.setShowDeleteConfirm(null)} className="w-full max-w-sm rounded-lg border border-main-border bg-white p-8 text-center shadow-[var(--shadow-overlay)]">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button type="button" disabled={ui.isProcessing} onClick={() => ui.setShowDeleteConfirm(null)} className="rounded-md border border-main-border py-2.5 text-sm font-medium text-main-text/70 transition-colors hover:bg-surface disabled:opacity-50">
                Batal
              </button>
              <button type="button" disabled={ui.isProcessing} onClick={() => ui.handleDelete(ui.showDeleteConfirm!)} className="rounded-md bg-red-600 py-2.5 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:opacity-50">
