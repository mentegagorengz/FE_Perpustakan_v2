...
import type { ApiBook } from "@/features/books/types/api";
import type { UiBook } from "@/features/books/types/ui";
...
export function useBorrow() {
  ...
  const availableCount = detail?.items?.filter((it) => it.status === "AVAILABLE").length ?? 0;
